...
class DataStorage:
    ...
    def store_metric(self, device_id, metric_data):
        """Armazena uma métrica recebida"""
        timestamp = datetime.now().isoformat()
        filename = os.path.join(self.base_dir, "metrics", f"{device_id}.json")
        
        data = {
            "timestamp": timestamp,
            "task_type": metric_data.task_type,
            "metric_value": metric_data.metric_value,
            "seq_num": metric_data.seq_num
        }
