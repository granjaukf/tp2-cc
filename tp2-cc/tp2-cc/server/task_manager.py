...
class TaskManager:
    ...
    def send_single_task(self, task_pdu, device_id, agent_addr, server_socket):
        ...
        task_pdu.seq_num = get_next_seq_num(device_id, len(task_pdu.pack()))
    ...
    def handle_alert(self, device_id, seq_num):
        """Marca um alerta como processado"""
        with self._lock:
            print(f"[DEBUG] Tentando processar alerta para (device_id={device_id}, seq_num={seq_num})")
            event = self._pending_alerts.get((device_id, seq_num))
            if event:
                event.set()  # Marca o evento de alerta como processado
                print(f"[DEBUG] Evento de alerta sinalizado para (device_id={device_id}, seq_num={seq_num})")
                return True
            else:
                print(f"[ERRO] Nenhum evento de alerta encontrado para (device_id={device_id}, seq_num={seq_num})")
        return False
